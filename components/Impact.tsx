...
export default function Impact() {
  ...
  return (
    <section id="impact" className="py-14 sm:py-24 md:py-36 border-t border-white/[0.06]">
      {/* Impact numbers */}
      <div className="px-5 sm:px-8 md:px-12 lg:px-16 mb-14 sm:mb-24 max-w-7xl mx-auto">
        ...
        <h2 className="font-display font-light text-[clamp(30px,7vw,70px)] leading-[0.95] text-cream tracking-[-0.01em] mb-16 max-w-lg">
          {imp.title[lang][0]}<br />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          {imp.items.map((item, i) => (
            <div key={i} className="border-t border-white/10 pt-6">
              <div className="font-display font-light text-[40px] sm:text-[52px] md:text-[60px] leading-none text-cream mb-1">
                {item.value}<span className="text-gold text-[22px] ml-1">{item.unit}</span>
              </div>
              <p className="font-sans text-[11px] sm:text-[13px] leading-[1.7] text-cream/85 mt-2">{t(item.label, lang)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="overflow-hidden border-y border-white/[0.06] py-px bg-charcoal">
        <div className="flex gap-0" style={{ width: "max-content", animation: "marquee 35s linear infinite" }}>
          {[...imp.testimonials, ...imp.testimonials].map((t2, i) => (
            <div key={i} className="w-[280px] sm:w-[340px] md:w-[420px] flex-shrink-0 px-5 sm:px-8 md:px-12 py-7 sm:py-10 border-r border-white/[0.06]">
              <p className="font-display font-light text-[14px] sm:text-[17px] leading-[1.65] text-cream/90 italic mb-6">
