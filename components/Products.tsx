...
export default function Products() {
  ...
  return (
    <section
      id='products'
      className='py-14 sm:py-24 md:py-36 px-5 sm:px-8 md:px-12 lg:px-16 border-t border-white/[0.06]'>
        ...
        <div className='flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16'>
          <div>
            ...
            <h2 className='font-display font-light text-[clamp(30px,7vw,72px)] leading-[0.95] text-cream tracking-[-0.01em]'>
              {p.title[lang][0]}
              <br />
              <em className='italic text-gold'>{p.title[lang][1]}</em>
              <br />
              {p.title[lang][2]}
            </h2>
          </div>
          <div className='text-right'>
            <p className='font-sans text-[12px] text-cream/80 italic mb-3 max-w-xs'>
              {t(p.subtitle, lang)}
            </p>
            <a
              href='#'
              className='font-mono text-[10px] tracking-[0.18em] uppercase text-cream/80 hover:text-cream flex items-center gap-3 transition-colors duration-300 group justify-end'>
              {t(p.viewAll, lang)}
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-white/[0.05]'>
          {p.items.map(product => (
            <div
              ...
              <div
                className={`aspect-[3/4] relative overflow-hidden bg-gradient-to-b ${bgMap[product.color]} to-charcoal`}>
                <div
                  className={`absolute inset-0 ${colorMap[product.color]} opacity-50 group-hover:opacity-70 transition-opacity duration-500`}>
                ...
                <div
                  className={`absolute inset-0 bg-ink/85 flex items-center justify-center p-5 transition-opacity duration-400 ${hoveredId === product.id ? 'opacity-100' : 'opacity-0'}`}>
                  <p className='font-sans text-[13px] leading-[1.75] text-cream/90 text-center'>
              ...
              <div className='p-5 border-t border-white/[0.06]'>
                <div className='flex justify-between items-start mb-2'>
                  <div>
                    <h3 className='font-sans font-medium text-[15px] sm:text-[17px] text-cream mb-0.5'>
                      {t(product.name, lang)}
                    </h3>
                    <div className='font-mono text-[11px] tracking-[0.15em] uppercase text-cream/85'>
                      {t(product.species, lang)}
                    </div>
                  </div>
                </div>
                <div className='flex items-center justify-between mt-4 pt-4 border-t border-white/[0.05]'>
                  <div className='font-mono text-[9px] tracking-[0.12em] uppercase text-sage'>
                    {t(product.region, lang)}
                  </div>
