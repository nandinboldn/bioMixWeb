      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          ...
          <div className="text-right">
            <p className="font-sans text-[12px] text-cream/40 italic mb-3 max-w-xs">{t(p.subtitle, lang)}</p>
            <a href="#" className="font-mono text-[10px] tracking-[0.18em] uppercase text-cream/40 hover:text-cream flex items-center gap-3 transition-colors duration-300 group justify-end">
              {t(p.viewAll, lang)}
            ...
            >
              <div className={`aspect-[3/4] relative overflow-hidden bg-gradient-to-b ${bgMap[product.color]} to-charcoal`}>
                ...
                <div className={`absolute inset-0 bg-ink/85 flex items-center justify-center p-5 transition-opacity duration-400 ${hoveredId===product.id ? "opacity-100" : "opacity-0"}`}>
                  <p className="font-sans text-[11px] leading-[1.7] text-cream/60 text-center">{t(product.description, lang)}</p>
                </div>
              </div>
              ...
              <div className="p-5 border-t border-white/[0.06]">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-sans font-medium text-[14px] text-cream mb-0.5">{t(product.name, lang)}</h3>
                    <div className="font-mono text-[9px] tracking-[0.15em] uppercase text-muted">{t(product.species, lang)}</div>
                  </div>
                </div>
