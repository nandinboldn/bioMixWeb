...
export default function Navbar() {
  ...
  return (
    <nav
      ...
      <Link
        href='/'
        className='font-sans font-extrabold text-[15px] tracking-[0.35em] text-cream uppercase flex items-center'>
        <Image
          alt='biomix logo'
          src={'/pictures/biomix_logo_final.png'}
          width={150}
          height={50}
        />
      </Link>
      ...
      <ul className='hidden md:flex items-center gap-10'>
        {links.map(l => (
          <li key={l.href}>
            <a
              href={l.href}
              className='font-mono text-[1em] tracking-[0.18em] uppercase text-cream/50 hover:text-cream transition-colors duration-300'>
              {l.label}
            </a>
          </li>
        ))}
      ...
      <div className='hidden md:flex items-center gap-3'>
        ...
        <div className='flex items-center border border-white/20 overflow-hidden'>
          <button
            onClick={() => setLang('mn')}
            className={`font-mono text-[1em] tracking-[0.12em] px-3 py-2 transition-all duration-200 ${
              lang === 'mn'
        ...
        <a
          href='#products'
          className='font-mono text-[10px] tracking-[0.15em] uppercase bg-cream text-ink px-6 py-3 hover:bg-gold transition-colors duration-300 font-light'>
          {t(c.nav.cta, lang)}
        </a>
      </div>
